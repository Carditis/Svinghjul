...
alpha_list = [(-Samlet.slope_efter*Start_omega-Samlet.intercept_efter)/Samlet.MaaltInerti]
tabtenergi_list = [0]
t_interval = 1/10
energi_list = []
...
while 1/2 * Samlet.MaaltInerti * omega_list[-1] ** 2 > Energigraense:
    ...
    alpha_list.append((-(Samlet.slope_efter * omega_list[i] + Samlet.intercept_efter))/Samlet.MaaltInerti)
    i += 1
...
ax1.plot(tid_list,energi_list, color="green")
...
ax1.set_ylabel('Kinetisk Energi [kJ]')
ax1.plot(omega_list,energi_list, color="blue")
ax1.tick_params(axis="y")
